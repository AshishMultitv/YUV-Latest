...
//        }
    ...
    func showredeemrefreemcodealert() {
        ...
        let saveAction = UIAlertAction(title: "Submit", style: .default, handler: { alert -> Void in
            ...
            else
            {
                ...
                MatomoTracker.shared.track(eventWithCategory: "Subscription", action: "Redeem_referral", name: couponTextField.text!, value: 1)
            }
    ...
    func clickoncell(sender:UIButton)
    {
        ...
        if(Common.Islogin())
        {
            ...
            if(pakeagename == "1 Day @ Free")
            {
                ...
                MatomoTracker.shared.track(eventWithCategory: "Subscription", action: "advertisement", name: "Subscription_oneday", value: 1)
    ...
    func showonetimesubscription(titile:String,description:String) {
        ...
        MatomoTracker.shared.track(eventWithCategory: "Subscription", action: "Subscription_initiate", name: "Iap", value: 1)
    ...
    func createSubscriptionorderid() {
       ...
       MatomoTracker.shared.track(eventWithCategory: "Subscription", action: "Subscription_Create", name: "Iap", value: 1)
        ...
        SwiftyStoreKit.purchaseProduct(Constant.getsubscriptionid(subscriptiontype: Subscriptiontype, subscriptionname: Subscriptioniapname, regiontype:LoginCredentials.Regiontype), atomically: true) { result in
            ...
            if case .success(let purchase) = result {
                ...
                print(purchase.productId)
                 MatomoTracker.shared.track(eventWithCategory: "Subscription", action: "Payment_sucess", name: "Iap", value: 1)
                Common.stoploderonsubscription(view: self.view)
            ...
            {
                print(eror.code)
              MatomoTracker.shared.track(eventWithCategory: "Subscription", action: "Payment_failed", name: "Iap", value: 1)
                Common.stoploderonsubscription(view: self.view)
    ...
    func VerifyPaymentourserver(tran_id:String,sub_id:Int,orderid:Int)
    {
        ...
        manager.post(url, parameters: Param, progress: nil, success: { (task: URLSessionDataTask, responseObject: Any?) in
            if (responseObject as? [String: AnyObject]) != nil {
                ...
                if(number == 0)
                {
                        MatomoTracker.shared.track(eventWithCategory: "Subscription", action: "Subscription_failed", name: "Iap", value: 0)
                    Common.Verifyfailedpayment()
                    LoginCredentials.Ispaymentfailedonsever = true
                }
                else
                {
                        MatomoTracker.shared.track(eventWithCategory: "Subscription", action: "Subscription_Sucess", name: "Iap", value: 1)
