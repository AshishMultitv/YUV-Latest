...
class StreamReader {
    ...
    func nextLine() -> String? {
        ...
        var range = buffer.range(of: delimData as Data, options: [], in: NSRange(location: 0, length: buffer.length))
        while range.location == NSNotFound {
            ...
            range = buffer.range(of: delimData as Data, options: [], in: NSRange(location: 0, length: buffer.length))
        }
