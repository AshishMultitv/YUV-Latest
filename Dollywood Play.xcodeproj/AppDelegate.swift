...
class AppDelegate: UIResponder, UIApplicationDelegate,UNUserNotificationCenterDelegate,OSSubscriptionObserver {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        NotificationCenter.default.addObserver(self,selector: #selector(userlogoutfromapp),name: NSNotification.Name(rawValue: "Userapplogout"),object: nil)
         ...
         NotificationCenter.default.addObserver(self,selector: #selector(Startuserappsession),name: NSNotification.Name(rawValue: "startusersession"),object: nil)
        
            NotificationCenter.default.addObserver(self,selector: #selector(Enduserappsession),name: NSNotification.Name(rawValue: "ensusersession"),object: nil)
    ...
    func Startuserappsession()
    {
        ...
        {
            let dict = dataBase.getDatabaseresponseinentity(entityname: "Logindata", key: "logindatadict")
              json = ["device_type":"ios","customer_id":(dict.value(forKey: "id") as! NSNumber).stringValue  ,"type":"start", "time":myStringafd,"device_unique_id" : uuid as String] as [String : Any]
        }
        else
        
        {
               json = ["device_type":"ios","customer_id":"0" ,"type":"start", "time":myStringafd,"device_unique_id" : uuid as String] as [String : Any]
        }
            print("Startuserappsession")
            print("json >>",json)
            var url = String(format: "%@%@", LoginCredentials.Csession,Apptoken)
            print(url)
            url = url.trimmingCharacters(in: .whitespaces)
            let manager = AFHTTPSessionManager()
            manager.post(url, parameters: json, progress: nil, success: { (task: URLSessionDataTask, responseObject: Any?) in
    ...
    func Enduserappsession()
    {
        ...
        {
            let dict = dataBase.getDatabaseresponseinentity(entityname: "Logindata", key: "logindatadict")
            json = ["device_type":"ios","customer_id":(dict.value(forKey: "id") as! NSNumber).stringValue  ,"type":"end", "time":myStringafd,"device_unique_id" : uuid as String] as [String : Any]
        }
        else
        {
            json = ["device_type":"ios","customer_id":"0" ,"type":"end", "time":myStringafd,"device_unique_id" : uuid as String] as [String : Any]
        }
        print("Enduserappsession")
        print("json >>",json)
        var url = String(format: "%@%@", LoginCredentials.Csession,Apptoken)
        print(url)
        url = url.trimmingCharacters(in: .whitespaces)
        ...
        manager.get(url, parameters: nil, progress: nil, success: { (task: URLSessionDataTask, responseObject: Any?) in
            if (responseObject as? [String: AnyObject]) != nil {
                ...
                else
                {
                    ...
                    //////////////////////////////dvr_url   34   //////////////////////////
                    let Csessionapi = Catdata_dict.value(forKey: "csession") as! String
                    let CsessionapiArr : [String] = Csessionapi.components(separatedBy: "|,")
                    LoginCredentials.Csession = CsessionapiArr[1]
                    
                    
                    //////////////////////////////dvr_url   34   //////////////////////////
                    let Feedbackapiapi = Catdata_dict.value(forKey: "feedback") as! String
                    let FeedbackapiapiArr : [String] = Feedbackapiapi.components(separatedBy: "|,")
                    let trimmedString = FeedbackapiapiArr[1].trimmingCharacters(in: .whitespaces)
                    LoginCredentials.Feedbackapi = trimmedString
